  ...
  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
    ...
    <Card className="p-4">
      <h3 className="text-sm font-bold text-gray-800 mb-3 flex items-center gap-2">
        <TrendingUp className="w-4 h-4 text-blue-500" /> Tóm tắt tài chính
      </h3>
      <div className="space-y-2">
        {[
          ...
          <div key={i} className={`flex justify-between items-center px-3 py-2 ${item.bg} rounded-xl`}>
            <span className="text-[11px] font-medium text-gray-700 flex items-center gap-2"><span>{item.icon}</span>{item.label}</span>
    ...
    <Card className="p-4">
      <h3 className="text-sm font-bold text-gray-800 mb-3 flex items-center gap-2">
        <ShoppingCart className="w-4 h-4 text-indigo-500" /> Thống kê đơn hàng
      </h3>
      <div className="grid grid-cols-2 gap-2 h-[calc(100%-2rem)]">
        {[
          { label: 'Tổng số đơn hàng trong kỳ', value: stats.totalOrders, icon: '📋', bg: 'bg-slate-50', colSpan: 'col-span-2' },
          ...
          { label: 'Chưa thanh toán', value: stats.unpaidCount, icon: '⏳', bg: 'bg-amber-50' },
        ].map((item, i) => (
          <div key={i} className={`p-3 ${item.bg} rounded-xl text-center flex flex-col items-center justify-center ${item.colSpan || ''}`}>
    ...
    <Card className="p-4 lg:col-span-2">
      <h3 className="text-sm font-bold text-gray-800 mb-3 flex items-center gap-2">
        <FileText className="w-4 h-4 text-slate-500" /> Giao dịch trong kỳ ({allTransactions.length})
      </h3>
